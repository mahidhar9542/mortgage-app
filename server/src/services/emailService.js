...
/**
 * Send refinance-specific welcome email to lead
 * @param {Object} lead - Lead document with refinance data
 * @returns {Promise<Object>} Email send result
 */
export const sendRefinanceLeadEmail = async (lead) => {
  const refinanceData = lead.refinanceData || {};
  const estimatedSavings = refinanceData.estimatedSavings || 0;
  
  return sendEmail({
    to: lead.email,
    subject: 'Your Refinance Application - Next Steps to Save Money',
    template: 'welcome-refinance',
    data: {
      lead: lead.toObject(),
      refinanceData,
...
/**
 * Send refinance-specific notification to admin
 * @param {Object} lead - Lead document with refinance data
 * @returns {Promise<Object>} Email send result
 */
export const sendRefinanceLeadAdminEmail = async (lead) => {
  const leadUrl = `${process.env.ADMIN_URL}/leads/${lead._id}`;
  const refinanceData = lead.refinanceData || {};
  
  return sendEmail({
    to: process.env.ADMIN_EMAIL || 'admin@example.com',
    subject: `New Refinance Lead: ${lead.firstName} ${lead.lastName} - Potential Savings: $${refinanceData.estimatedSavings?.toLocaleString() || 0}`,
    template: 'new-refinance-lead-admin',
    data: {
      lead: {
        ...lead.toObject(),
        leadUrl
      ...
      refinanceData,
      subject: `New Refinance Lead: ${lead.firstName} ${lead.lastName}`
    }
  });
};
